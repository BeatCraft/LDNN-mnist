...
def main():
    ...
    batch_offset =  batch_size * batch_offset_index
    # data
    data_array = BATCH_DATA_ARRAY[batch_offset:(batch_offset + batch_size)]
    avg = 0.0
    bsum = 0.0
    std = 0.0
    std2 = 0.0
    for i in range(batch_size): # scale to 0.0 - 1.0
        data_array[i] = data_array[i] / 255.0
        bsum = bsum + data_array[i].sum()
    #
    avg = bsum / float(data_size*batch_size)
    bsum = 0.0
    for i in range(batch_size):
        for p in range(data_size):
            t = data_array[i][p] - avg
            bsum = bsum + t * t
        #
    #
    std2 = bsum / float(batch_size)
    r.direct_set_data(data_array)
    std = math.sqrt(std2)
    print(avg, std)
    ...
    for i in range(batch_size):
        for p in range(data_size):
            xi = (data_array[i][p] - avg) / std
            #bsum = bsum + t * t
            data_array[i][p] = xi/2.0 + 0.5
        #
    #
    #print(data_array[i])
    #return 0
    ...
    # label
    labels = LABEL_ARRAY[batch_offset:(batch_offset + batch_size)]
    ...
    t = train.Train(r)
    t.w_list = t.make_w_list() #w_list[idx] #t.make_w_list()
    t.main_simple_loop(batch_offset_index, ce, 1000, 4) # 16
    
    #ce = mini_train(r, w_list, batch_size, 0, ce, 1)
    return 0
    
    for k in range(100):
        for i in range(batch_num):
            ce = mini_train(r, w_list, batch_size, i, ce, 100)
            print(ce)
        #
    #
    return 0
